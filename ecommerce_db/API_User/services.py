...
class RecommendationService:
    ...
    def load_model(self, model_filename):
        ...
        if not os.path.exists(pkl_file_path):
            ...
            print(f" Run: python API_User/setup_volume.py to download models")
            return None
        
        try:
            file_size = os.path.getsize(pkl_file_path) / (1024*1024)  # MB
            print(f"Loading {model_filename} ({file_size:.1f} MB)")
            ...
            print(f" Error loading {model_filename}: {e}")
            print(f" Model may be corrupted. Run: python API_User/setup_volume.py")
            return None
